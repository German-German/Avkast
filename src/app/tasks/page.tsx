          ...
          {!loading && data && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              <MetricCard 
                ...
                value={`$${data.portfolio_summary.total_value_usd.toLocaleString()}`} 
              />
              <MetricCard 
                label="ESG Alignment" 
                value={`${data.portfolio_summary.esg_alignment_score}/100`} 
              />
              <MetricCard 
                label="Active Signals" 
                value={data.active_signals.length.toString()} 
              />
              <MetricCard 
                label="Risk Profile" 
                value={data.portfolio_summary.risk_profile} 
              />
            </div>
          )}
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 gap-4">
                {data?.active_signals.map((task: Task) => (
                  <div key={task.id} className="p-6 rounded-xl glass border border-white/5 hover:border-primary/20 transition-all group">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-3">
                        <button className="px-5 py-2 rounded-md bg-accent text-accent-foreground text-xs font-bold uppercase tracking-tight hover:opacity-90 transition-opacity">
                          APPROVE
                        </button>
                        <button className="px-5 py-2 rounded-md bg-white/5 border border-white/10 text-foreground text-xs font-bold uppercase tracking-tight hover:bg-white/10 transition-all">
                          REJECT
                        </button>
                      </div>
                    </div>
